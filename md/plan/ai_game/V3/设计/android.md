...
首先说明一下这个是游玩中界面和调试界面共用一个业务逻辑的界面。
会有台词，会有语音，无论是主页还是游玩界面都需要用户输入过文字
才算“聊过”。用户点击标3，标5，标6都会进入游玩界面。
标1：手机自己自带的信息区域包含电量等。后面不再复述
标2: 快捷按键区：故事大厅，语音开关（控制主页是否有声音，缓存下次打开主页时控制是否有语音）
标3：占据整个界面的第一章节背景图
标4：开场白，角色对话 等自动播放到用户输入为止
标5：故事标题区
标6：灰色的用户输入区。点击会进入游玩界面
标7：菜单栏区。

# 特别说明
无论是主页还是“我的”还是聊过点击进入一个聊过的故事
都不应该重新播放而是从上次进度开始继续进行！！！！
...
上传的头像可以是png 也可以是gif , 但是尺寸一定要标准化。
